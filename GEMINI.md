...
- **Database:** Google Cloud Firestore (via `packages/db`).
- **AI Orchestration:** Gemini Pro/Flash (via `packages/capabilities`).
...
| `apps/web/` | Web Frontend | Next.js only. **Ignore** `pyproject.toml` or `backend/` remnants here. |
| `apps/admin/` | Admin Dashboard | Next.js frontend for internal workflows. |
| `packages/db/` | Data Access Layer | Centralized Firestore/BigQuery logic. |
| `packages/common-python/` | Shared Logic | Source of truth for Pydantic models (`hephae_common.models`). |
| `packages/capabilities/` | AI Agents | Individual agent logic (SEO, Margin, etc.) using ADK. |
| `contracts/` | Specifications | Markdown schemas that MUST be reflected in Pydantic models. |
...
- **Schema Alignment:** Before changing `hephae_common.models`, verify the change against `contracts/firestore-schema.md`.

### AI & LLM Orchestration
- **ADK Usage:** All "capabilities" must be implemented as `google-adk` agents or runners in `packages/capabilities/`.
...
### Build Systems
- **Standard:** Use `hatch` as the primary Python build backend. 
...
1. Define the schema in `contracts/`.
2. Create Pydantic models in `hephae_common.models`.
3. Implement the agent in `packages/capabilities/hephae_capabilities/<name>/`.
4. Create an evaluation in `tests/evals/<name>/` using `*.test.json` files.
5. Register a router in `apps/api/backend/routers/`.
...
2. **Model Consolidation:** Migrate remaining models from `apps/api/backend/types.py` to `hephae_common.models`.
3. **Infrastructure Unification:** Centralize `infra/` configurations to prevent drift between Web and Admin deployments.
4. **Build Cleanup:** Fully remove legacy backend artifacts from `apps/web`.
