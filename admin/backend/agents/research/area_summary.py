...
async def generate_area_summary(
    ...
    result = await run_agent_to_json(AreaSummaryAgent, prompt, app_name="area_summary")
...
async def generate_enhanced_area_summary(
    ...
    result = await run_agent_to_json(EnhancedAreaSummaryAgent, prompt, app_name="enhanced_area_summary")
    if not result or not isinstance(result, dict):
