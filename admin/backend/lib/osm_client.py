...
_SEARCH_RADIUS_M = 2000  # 2 km radius

# Throttle Nominatim to 1 req/sec (their policy)
...
def _build_overpass_query(lat: float, lng: float, radius: int = _SEARCH_RADIUS_M) -> str:
    """Build Overpass QL query for businesses around a coordinate."""
    return f"""
[out:json][timeout:25];
(
...
def _parse_category(tags: dict[str, str]) -> str:
    """Extract a human-readable category from OSM tags."""
...
async def _geocode_zipcode(zip_code: str, client: httpx.AsyncClient) -> tuple[float, float] | None:
    """Geocode a US zip code to lat/lng via Nominatim."""
    ...
    if resp.status_code != 200:
        logger.warning(f"[OSM] Nominatim returned {resp.status_code} for {zip_code}")
        return None
    data = resp.json()
    if not data:
        logger.warning(f"[OSM] No geocode result for {zip_code}")
        return None
    return float(data[0]["lat"]), float(data[0]["lon"])


async def _query_overpass(lat: float, lng: float, client: httpx.AsyncClient) -> list[dict]:
    """Query Overpass API for businesses near a coordinate."""
    ...
    if resp.status_code != 200:
        logger.warning(f"[OSM] Overpass returned {resp.status_code}")
        return []
...
async def discover_businesses(zip_code: str) -> list[OsmBusiness]:
    ...
    Flow: geocode zip → query Overpass → parse results.
    """
