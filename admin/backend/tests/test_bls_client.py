...
from backend.lib.bls_client import (
...
class TestQueryBlsCpi:
    ...
    async def test_success(self):
        ...
        with patch("backend.lib.bls_client.settings") as mock_settings, \
             patch("backend.lib.bls_client.httpx.AsyncClient", return_value=mock_client), \
             patch("backend.lib.bls_client.get_cached_food_prices", new_callable=AsyncMock, return_value=None), \
             patch("backend.lib.bls_client.save_food_prices_cache", new_callable=AsyncMock):
    ...
    async def test_cache_hit_skips_api_call(self):
        ...
        with patch("backend.lib.bls_client.settings") as mock_settings, \
             patch("backend.lib.bls_client.get_cached_food_prices", new_callable=AsyncMock, return_value=cached_data), \
             patch("backend.lib.bls_client.httpx.AsyncClient") as mock_http:
            mock_settings.BLS_API_KEY = "test-key"
            result = await query_bls_cpi("pizza")
