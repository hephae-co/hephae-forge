...
class TestCpiEndpoint:
    def test_get_cpi(self, client):
        ...
        with patch("backend.routers.food_prices.query_bls_cpi", new_callable=AsyncMock, return_value=mock_data):
...
class TestCommoditiesEndpoint:
    def test_get_commodities(self, client):
        ...
        with patch("backend.routers.food_prices.query_usda_prices", new_callable=AsyncMock, return_value=mock_data):
...
class TestSummaryEndpoint:
    def test_get_summary(self, client):
        ...
        with patch("backend.routers.food_prices.query_bls_cpi", new_callable=AsyncMock, return_value=bls_data), \
             patch("backend.routers.food_prices.query_usda_prices", new_callable=AsyncMock, return_value=usda_data):
    ...
    def test_summary_with_empty_data(self, client):
        with patch("backend.routers.food_prices.query_bls_cpi", new_callable=AsyncMock, return_value=BlsCpiData()), \
             patch("backend.routers.food_prices.query_usda_prices", new_callable=AsyncMock, return_value=UsdaPriceData()):
