    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Industry-specific research pipeline for Restaurants, Barbers, and Bakeries</p>
        </div>
        <button
          onClick={fetchStats}
          disabled={loading}
      ...
      {stats && (
        <>
          ...
          {/* Businesses */}
          <div>
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Businesses</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <StatCard
                icon={Store}
                label="Total Businesses"
                value={stats.businesses?.totalBusinesses ?? 0}
                subtitle={`${stats.businesses?.uniqueZipCodes ?? 0} zip codes`}
                accent="border-l-blue-500"
              />
              <StatCard
                icon={Building2}
                label="Discovered"
                value={stats.businesses?.discovered ?? 0}
                subtitle="Profile enriched"
                accent="border-l-emerald-500"
              />
              <StatCard
                icon={BarChart3}
                label="Fully Analyzed"
                value={stats.businesses?.analyzed ?? 0}
                subtitle="All agents run"
                accent="border-l-indigo-500"
              />
            </div>
          </div>
