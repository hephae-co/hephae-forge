...
from hephae_db.schemas import ContextCombinerOutput
...
async def combine_research_context(reports: list[dict]) -> dict:
    ...
    result = await run_agent_to_json(
        ContextCombinerAgent, prompt, app_name="context_combiner", response_schema=ContextCombinerOutput
    )
    if result and isinstance(result, ContextCombinerOutput):
