...
from hephae_db.schemas import IndustryNewsOutput
...
async def research_industry_news(industry: str, area: str) -> dict:
    ...
    result = await run_agent_to_json(
        ...
        response_schema=IndustryNewsOutput,
    )
    if result and isinstance(result, IndustryNewsOutput):
        return result.model_dump()
    return {"recentNews": [], "priceTrends": [], "regulatoryUpdates": []}
