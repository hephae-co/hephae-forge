...
GET    /api/ai-tool-discovery/{vertical}/latest      — Latest run for a vertical (full tools list)
GET    /api/ai-tool-discovery/{vertical}/{week_of}   — Specific run
...
async def list_runs(
    ...
    from hephae_db.firestore.ai_tool_discovery import list_ai_tool_runs

    runs = await list_ai_tool_runs(vertical=vertical, limit=limit, summaries_only=True)
...
async def get_latest(vertical: str):
    ...
    from hephae_db.firestore.ai_tool_discovery import get_latest_ai_tool_run
...
async def get_run(vertical: str, week_of: str):
    ...
    from hephae_db.firestore.ai_tool_discovery import get_ai_tool_run
...
async def delete_run(vertical: str, week_of: str):
    """Delete a specific discovery run."""
    from hephae_db.firestore.ai_tool_discovery import delete_ai_tool_run

    doc_id = f"{vertical}-{week_of}"
    await delete_ai_tool_run(doc_id)
    return {"success": True}
