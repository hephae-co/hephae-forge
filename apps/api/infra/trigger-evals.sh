...
#
# Usage:
#   bash apps/api/infra/trigger-evals.sh                       # all agents, static evals
#   bash apps/api/infra/trigger-evals.sh --agent seo_auditor   # single agent
#   bash apps/api/infra/trigger-evals.sh --human-curated        # Firestore-backed evals
#   bash apps/api/infra/trigger-evals.sh --unit-tests           # fast mocked unit tests (no LLM cost)
#   bash apps/api/infra/trigger-evals.sh --status               # check last execution
#
# Prerequisites:
#   1. Build & push the image:  bash apps/api/infra/deploy.sh
#   2. Create the job:          bash apps/api/infra/deploy-eval-job.sh
# ──────────────────────────────────────────────────────────────
set -euo pipefail
...
if ! gcloud run jobs describe "$JOB_NAME" \
    ...
    fail "Job '${JOB_NAME}' not found. Run: bash apps/api/infra/deploy-eval-job.sh"
fi
...
if $UNIT_TESTS; then
    ...
    UNIT_DIRS="/app/tests/db /app/tests/workflows /app/tests/api /app/tests/capabilities /app/tests/integrations"
