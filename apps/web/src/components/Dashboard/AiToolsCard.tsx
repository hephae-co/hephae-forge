...
export function AiToolsCard({ tools }: { tools: AiTool[] | null | undefined }) {
  if (!tools?.length) {
            ...
            <div className="flex items-center gap-2.5 mb-2">
              ...
              {t.url ? (
                <a href={t.url} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-slate-800 group-hover:text-purple-700 transition-colors flex items-center gap-1">
                  {toolName} <ExternalLink className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                </a>
              ) : (
