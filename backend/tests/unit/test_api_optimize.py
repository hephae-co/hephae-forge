...
async def client():
    ...
    with (
        patch("backend.routers.optimize.run_optimizer", new_callable=AsyncMock, return_value={"status": "ok", "run_at": "2026-03-03T00:00:00Z", "duration_seconds": 1.0}),
        patch("backend.routers.optimize._run_prompt_optimizer", new_callable=AsyncMock, return_value={"status": "ok"}),
        patch("backend.routers.optimize._run_ai_cost_optimizer", new_callable=AsyncMock, return_value={"status": "ok"}),
        patch("backend.routers.optimize._run_cloud_cost_optimizer", new_callable=AsyncMock, return_value={"status": "ok"}),
        patch("backend.routers.optimize._run_performance_optimizer", new_callable=AsyncMock, return_value={"status": "ok"}),
    ):
