"""
Unit tests for backend.lib.latest_outputs — shared Firestore latestOutputs fetcher.
"""
...
class TestFetchLatestOutputs:
    ...
    def test_returns_outputs_for_valid_business(self):
        with patch("backend.lib.db.read_business.read_business", return_value=SAMPLE_BUSINESS_DOC):
    ...
    def test_returns_empty_for_missing_business(self):
        with patch("backend.lib.db.read_business.read_business", return_value=None):
            from backend.lib.latest_outputs import fetch_latest_outputs
    ...
    def test_returns_empty_for_business_without_outputs(self):
        ...
        with patch("backend.lib.db.read_business.read_business", return_value=doc_no_outputs):
            from backend.lib.latest_outputs import fetch_latest_outputs
    ...
    def test_handles_exception_gracefully(self):
        with patch(
            "backend.lib.db.read_business.read_business",
    ...
    def test_uses_slug_for_lookup(self):
        with patch("backend.lib.db.read_business.read_business", return_value=None) as mock_read:
            from backend.lib.latest_outputs import fetch_latest_outputs
