"""BigQuery reader — Google Trends public dataset queries (from admin)."""
...
async def query_google_trends(dma_name: str) -> dict[str, Any]:
    """Query Google Trends BigQuery public dataset for a DMA region.

    Returns dict with topTerms and risingTerms lists.
    """
    empty: dict[str, Any] = {"topTerms": [], "risingTerms": []}
    if not dma_name:
        return empty
    ...
    try:
        ...
        return {
            "topTerms": [
                {"term": r["term"], "rank": r["rank"], "score": r["score"], "week": str(r.get("week", ""))}
                for r in top_rows
            ],
            "risingTerms": [
                {
