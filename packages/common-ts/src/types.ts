/**
 * Shared type definitions for hephae apps.
 *
 * These are the core API types that flow between web and admin.
 * Web app produces them (API responses), admin consumes them.
 * The Python Pydantic models in packages/common-python/hephae_common/models.py
 * are the source of truth — keep these in sync.
 */
