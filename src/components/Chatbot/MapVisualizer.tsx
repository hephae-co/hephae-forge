...
export default function MapVisualizer({ lat, lng, businessName, business, isDiscovering = false }: MapVisualizerProps) {
    ...
    const [activeTab, setActiveTab] = useState<'profile' | 'menu' | 'competitors'>('profile');
                    ...
                    <div className="bg-slate-900/85 backdrop-blur-xl border border-white/10 shadow-2xl rounded-2xl overflow-hidden">
                        ...
                        <div className="p-6 pb-4 border-b border-white/10">
                            ...
                            <div className="flex gap-1 p-1 bg-black/40 rounded-lg">
                                <button onClick={() => setActiveTab('profile')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${activeTab === 'profile' ? 'bg-indigo-500 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>Profile</button>
                                <button onClick={() => setActiveTab('menu')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${activeTab === 'menu' ? 'bg-indigo-500 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>Menu</button>
                                <button onClick={() => setActiveTab('competitors')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${activeTab === 'competitors' ? 'bg-indigo-500 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>Rivals</button>
                            </div>
                        </div>
                        ...
                        <div className="p-6 pt-4 space-y-4">
                            ...
                            {activeTab === 'menu' && (
                                <div className="animate-fade-in relative min-h-[140px] flex flex-col items-center justify-center text-center">
                                    {isDiscovering ? (
                                        <div className="flex flex-col items-center justify-center space-y-2 h-full py-4">
                                            <div className="flex space-x-1">
                                                <div className="w-2 h-2 rounded-full bg-indigo-500 animate-bounce" style={{ animationDelay: '0ms' }}></div>
                                                <div className="w-2 h-2 rounded-full bg-indigo-500 animate-bounce" style={{ animationDelay: '150ms' }}></div>
                                                <div className="w-2 h-2 rounded-full bg-indigo-500 animate-bounce" style={{ animationDelay: '300ms' }}></div>
                                            </div>
                                            <p className="text-slate-400 text-sm">Parsing digital assets...</p>
                                        </div>
                                    ) : (business as EnrichedProfile).menuScreenshotBase64 ? (
                                        <div className="w-full">
                                            <div className="h-32 rounded-xl overflow-hidden border border-white/10 mb-3 relative">
                                                <img
                                                    src={`data:image/jpeg;base64,${(business as EnrichedProfile).menuScreenshotBase64}`}
                                ...
                                <div className="space-y-3 animate-fade-in relative min-h-[140px]">
                                    {isDiscovering ? (
                                        <div className="flex flex-col items-center justify-center h-full space-y-2 py-4">
                                            ...
                                            <p className="text-slate-400 text-sm">Scanning geographic topology...</p>
                                        </div>
                                    ) : (business as EnrichedProfile).competitors?.length ? (
                                        <div className="space-y-2">
                                            {(business as EnrichedProfile).competitors!.map((comp, i) => (
