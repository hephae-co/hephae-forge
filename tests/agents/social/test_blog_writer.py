...
class TestBuildDataContext:
    ...
    def test_includes_business_name(self):
        from hephae_agents.social.blog_writer.agent import _build_data_context
...
class TestBlogParseJson:
    ...
    def test_plain_json(self):
        from hephae_agents.social.blog_writer.agent import _parse_json
...
class TestGenerateBlogPost:
    ...
    async def test_returns_expected_keys(self):
        with (
            patch("hephae_agents.social.blog_writer.agent.InMemorySessionService") as mock_svc_cls,
            patch("hephae_agents.social.blog_writer.agent.Runner") as mock_runner_cls,
        ):
...
class TestAgentConfig:
    ...
    def test_report_type_labels(self):
        from hephae_agents.social.blog_writer.agent import REPORT_TYPE_LABELS
...
class TestPickHeroStats:
    ...
    def test_prefers_margin_leakage(self):
        from hephae_api.routers.web.blog import _pick_hero_stats
...
async def blog_client():
    with (
        patch(
            "hephae_api.routers.web.blog.generate_blog_post",
            new_callable=AsyncMock,
            return_value=SAMPLE_BLOG_RESULT,
        ) as mock_blog,
        patch(
            "hephae_api.routers.web.blog.generate_universal_social_card",
            new_callable=AsyncMock,
            return_value=SAMPLE_HERO_BYTES,
        ) as mock_card,
        patch(
            "hephae_api.routers.web.blog.upload_report",
            new_callable=AsyncMock,
            return_value="https://storage.googleapis.com/everything-hephae/test/blog-1234.html",
        ) as mock_upload,
        patch("hephae_api.routers.web.blog.build_blog_report", return_value="<html>blog</html>") as mock_template,
        patch("hephae_api.routers.web.blog.write_agent_result", new_callable=AsyncMock) as mock_write,
    ):
...
class TestBlogRouterValidation:
    ...
    async def test_404_no_analysis_data(self, blog_client):
        with patch(
            "hephae_api.routers.web.blog.fetch_latest_outputs",
...
class TestPromptConstants:
    ...
    def test_research_compiler_instruction_exists(self):
        from hephae_agents.social.blog_writer.prompts import RESEARCH_COMPILER_INSTRUCTION
    ...
    def test_blog_writer_instruction_exists(self):
        from hephae_agents.social.blog_writer.prompts import BLOG_WRITER_INSTRUCTION
...
class TestBlogPostResponse:
    ...
    def test_default_values(self):
        from hephae_api.types import BlogPostResponse
