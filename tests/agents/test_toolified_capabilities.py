...
async def test_get_capability_tool_seo():
    ...
    with patch("hephae_api.workflows.capabilities.registry.get_capability") as mock_get:
        mock_cap = MagicMock()
        mock_cap.name = "seo"
...
def test_get_all_capability_tools():
    ...
    with patch("hephae_api.workflows.capabilities.registry.get_enabled_capabilities") as mock_list:
        mock_cap = MagicMock()
        mock_cap.name = "test"
        mock_list.return_value = [mock_cap]

        with patch("hephae_api.workflows.capabilities.tools.get_capability_tool") as mock_get_tool:
