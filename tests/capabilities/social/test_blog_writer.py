...
class TestBuildDataContext:
    ...
    def test_includes_business_name(self):
        from hephae_capabilities.social.blog_writer.agent import _build_data_context
...
class TestBlogParseJson:
    ...
    def test_plain_json(self):
        from hephae_capabilities.social.blog_writer.agent import _parse_json
...
class TestAgentConfig:
    ...
    def test_research_compiler_uses_primary_model(self):
        from hephae_capabilities.social.blog_writer.agent import research_compiler_agent
    ...
    def test_blog_writer_uses_enhanced_model(self):
        from hephae_capabilities.social.blog_writer.agent import blog_writer_agent
        from backend.config import AgentModels
    ...
    def test_report_type_labels(self):
        from hephae_capabilities.social.blog_writer.agent import REPORT_TYPE_LABELS
...
class TestPickHeroStats:
    ...
    def test_prefers_margin_leakage(self):
        from backend.routers.web.blog import _pick_hero_stats
...
async def blog_client():
    with (
        patch(
            "backend.routers.web.blog.generate_blog_post",
            new_callable=AsyncMock,
            return_value=SAMPLE_BLOG_RESULT,
        ) as mock_blog,
        patch(
            "backend.routers.web.blog.generate_universal_social_card",
            new_callable=AsyncMock,
            return_value=SAMPLE_HERO_BYTES,
        ) as mock_card,
        patch(
            "backend.routers.web.blog.upload_report",
            new_callable=AsyncMock,
            return_value="https://storage.googleapis.com/everything-hephae/test/blog-1234.html",
        ) as mock_upload,
        patch("backend.routers.web.blog.build_blog_report", return_value="<html>blog</html>") as mock_template,
        patch("backend.routers.web.blog.write_agent_result", new_callable=AsyncMock) as mock_write,
    ):
...
class TestBlogRouterValidation:
    ...
    async def test_404_no_analysis_data(self, blog_client):
        with patch(
            "backend.routers.web.blog.fetch_latest_outputs",
...
class TestPromptConstants:
    ...
    def test_research_compiler_instruction_exists(self):
        from hephae_capabilities.social.blog_writer.prompts import RESEARCH_COMPILER_INSTRUCTION
    ...
    def test_blog_writer_instruction_exists(self):
        from hephae_capabilities.social.blog_writer.prompts import BLOG_WRITER_INSTRUCTION
