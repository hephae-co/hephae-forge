...
from hephae_db.context.business_context import (
    BusinessContext,
...
class TestBuildBusinessContext:
    ...
    async def test_merges_stored_business_data(self):
        ...
        with (
            patch("hephae_db.context.business_context.read_business", return_value=stored),
            patch("hephae_db.context.business_context.get_zipcode_report", new_callable=AsyncMock, return_value=None),
            patch("hephae_db.context.business_context.get_area_research_for_zip", new_callable=AsyncMock, return_value=None),
        ):
            ctx = await build_business_context(SAMPLE_IDENTITY, capabilities=["traffic"])
    ...
    async def test_loads_admin_data(self):
        ...
        with (
            patch("hephae_db.context.business_context.read_business", return_value=None),
            patch("hephae_db.context.business_context.get_zipcode_report", new_callable=AsyncMock, return_value=zip_report),
            patch("hephae_db.context.business_context.get_area_research_for_zip", new_callable=AsyncMock, return_value=area_report),
        ):
