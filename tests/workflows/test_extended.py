...
from backend.workflows.agents.outreach.communicator import draft_and_send_outreach, CommunicatorInput
from backend.workflows.test_runner import test_runner
from backend.workflows.agents.evaluators.seo_evaluator import seo_evaluator_agent

@pytest.mark.asyncio
@patch("backend.workflows.agents.outreach.firestore_service.get_business_by_id")
@patch("google.adk.Runner.run_async")
@patch("backend.workflows.agents.outreach.send_email", new_callable=AsyncMock)
...
@patch("backend.workflows.test_runner.HephaeAdminRunner.evaluate_with_agent", new_callable=AsyncMock)
