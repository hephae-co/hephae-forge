...
async def client():
    ...
    with (
        patch("backend.routers.capabilities.competitive.InMemorySessionService", return_value=mock_session_svc),
        patch("backend.routers.capabilities.competitive.Runner", side_effect=_make_runner),
        patch("backend.routers.capabilities.competitive.upload_report", new_callable=AsyncMock, return_value="https://storage.googleapis.com/test/competitive.html"),
        patch("backend.routers.capabilities.competitive.build_competitive_report", return_value="<html>comp</html>"),
        patch("backend.routers.capabilities.competitive.generate_slug", side_effect=lambda n: n.lower().replace(" ", "-")),
        patch("backend.routers.capabilities.competitive.write_agent_result", new_callable=AsyncMock, return_value=None),
        patch("backend.routers.capabilities.competitive.generate_and_draft_marketing_content", new_callable=AsyncMock, return_value=None),
    ):
...
def _setup_two_step(client, profiler_text: str, positioning_text: str):
    ...
    import backend.routers.capabilities.competitive as mod
