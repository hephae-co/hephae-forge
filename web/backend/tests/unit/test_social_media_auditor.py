...
class TestAgentConfig:
    def test_researcher_has_tools(self):
        from backend.agents.social_media_auditor import social_researcher_agent
    ...
    def test_strategist_has_no_tools(self):
        from backend.agents.social_media_auditor import social_strategist_agent
    ...
    def test_strategist_uses_thinking(self):
        from backend.agents.social_media_auditor import social_strategist_agent
        from backend.config import ThinkingPresets
    ...
    def test_agents_have_fallback(self):
        from backend.agents.social_media_auditor import social_researcher_agent, social_strategist_agent
...
class TestSocialAuditReportModel:
    def test_minimal_valid(self):
        from backend.types import SocialAuditReport
...
async def client():
    ...
    with (
        patch("backend.routers.capabilities.marketing.InMemorySessionService", return_value=mock_session_svc),
        patch("backend.routers.capabilities.marketing.Runner", side_effect=_make_runner),
        patch("backend.routers.capabilities.marketing.upload_report", new_callable=AsyncMock, return_value="https://storage.googleapis.com/test/social-audit.html"),
        patch("backend.routers.capabilities.marketing.build_social_audit_report", return_value="<html>audit</html>"),
        patch("backend.routers.capabilities.marketing.generate_slug", side_effect=lambda n: n.lower().replace(" ", "-")),
        patch("backend.routers.capabilities.marketing.write_agent_result", new_callable=AsyncMock, return_value=None),
    ):
...
def _setup_two_step(client, researcher_text: str, strategist_text: str):
    ...
    import backend.routers.capabilities.marketing as mod
